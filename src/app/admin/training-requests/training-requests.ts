...
import { TrainingRequest, Trainer } from '../../shared/models';
import { TrainingRequestService, TrainerService, EnrollmentService } from '../../shared/services';
...
export class TrainingRequestsComponent implements OnInit {
  ...
  assignTrainer(requestId: number, trainerId: number | null) {
    ...
    this.enrollmentService.create(enrollmentData).subscribe({
      next: () => {
        this.successMessage.set('Trainer assigned successfully! Enrollment created.');
        setTimeout(() => this.successMessage.set(null), 3000);
        this.loadRequests();
      },
      error: (err) => {
